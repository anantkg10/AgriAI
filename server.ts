...
const PORT = 3000;
const FALLBACK_KEY = "AIzaSyBEqysd-OroYv_s4h3ez1sGbMnY-7OOekA";

function getApiKey(): string {
  return (process.env.GEMINI_API_KEY && process.env.GEMINI_API_KEY !== "PLACEHOLDER_API_KEY")
...
6. List 2-3 actionable prevention tips with short descriptions.`;
      ...
      res.status(500).json({ error: error.message || "Failed to analyze plant image" });
    }
  });
...
${JSON.stringify(textToTranslate, null, 2)}`;
      ...
      res.status(500).json({ error: error.message || "Failed to translate analysis results" });
    }
  });
